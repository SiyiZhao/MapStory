...
- 时间排序规则：精确日期（如 `2020-01-01`）> 模糊时间（如 `2020`、`2020-01`）> 冲突时间（无法解析格式）> 空时间。
- 经纬度会进行范围校验：`lat` 必须在 `[-90, 90]`，`lon` 必须在 `[-180, 180]`。
- `list --limit` 必须为正整数；`search --lat-range/--lon-range` 支持自动纠正反向区间（如 `40 20` 会按 `20 40` 处理）。
