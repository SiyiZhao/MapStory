...
```
query_by_location 比起经纬度 query，可能用现今行政区划名 query 更方便。但也可以提供经纬度 query，配合地理编码工具（如 Nominatim）将地名转换为经纬度进行查询。
...
```
——有待细化，导出的优先级不高，但是除了导出还应有导入功能，我没看到在哪层实现？
---
...
#### 1.1 时间排序功能 ✗
- **目标**: 实现按时间排序的查询输出
- **实现**: 在 `mapstory/store.py` 中添加 `list_all(sort_by="time")` 方法
...
- **实现**:
  ...
  - 在 `mapstory/time_utils.py` 中创建年号查询接口
  - 支持按年号查询所对应的公元纪年 ——注意中国历史上的“年”并不与公元纪年完全重合，我们的数据库希望尽可能准确地记录年号变更的日期。
...
- **用户文档**: 更新 `docs/USAGE.md` 和 `docs/user-preferences.md`——后者是给AI看的，是否应该更名？
