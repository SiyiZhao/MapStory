...
- **目标**: 提供易用的本地自托管 Web 应用。
- **当前实现**:
  - 创建 `mapstory/ext/web.py` 中的 Flask 应用工厂 `create_app()`。
  - 使用现有 `EventStore` 和 SQLite 数据库，不额外引入 ORM 层。
  - 提供事件列表、筛选、新增、编辑、删除、详情页。
  - 提供与页面语义一致的 JSON API：`/api/events` 与 `/api/events/<id>`。
- **前端**:
  - 使用 HTML/CSS 实现响应式单页式布局。
  - 当前版本先交付列表、详情和表单，不引入地图组件。
- **部署**: 本地运行即可，也可按需开放到局域网。
- **优先级**: 低（但已有 MVP 可用）。
