...
## v1.2.0 (2026-05-28)
- Web 轨迹地图新增内置离线矢量底图数据与底图切换：默认显示现代行政规划图，可切换为秦末汉初的当时地理图。
- 底图层新增权威图片插槽：`data/basemaps/modern.*` 可使用自然资源部/天地图标准地图，`data/basemaps/qin-han-liuhou.*` 可使用合法取得的《中国历史地图集》秦汉相关图幅。

## v1.1.0 (2026-05-27)
- Web UI 新增事件轨迹地图页 `/events/map`，支持按人物、地点、关键词和年份范围筛选后，以时间顺序连接带坐标的事件。
- 新增《史记·留侯世家》张良主线事件种子数据，可写入本地事件库后直接绘制人生地理轨迹。
- 补充地图页回归测试，覆盖按人物筛选与轨迹渲染。

## v1.0.1 (2026-04-04)
- Web 详情页显示调整：事件描述显示在标题，备注框增大。
- Web 编辑页显示调整：事件描述框缩小，备注框增大。
- Web 列表页显示调整：时间备注挪到上方，显示“备注/出处”信息；将“自设”优先级颜色调整为蓝色，便于与“史实（存疑）/史实（删减）”的警示色区分。

## v1.0.0 (2026-04-04)
- 使用 Claude 重新设计仓库结构与开发流程。
- 使用 Codex 重新编写仓库核心代码与模块实现。
- 实现并交付基于 Flask 的 Web UI（事件列表、筛选、详情与编辑能力）。
